...
const BookingInformation: React.FC<ProcessPage> = ({
    ...
    const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        updateFormData({ consultationDate: date, consultationTime: '', schedularUuid: '' });
        fetchTimeSlots(date);
    };
    ...
    return (
        <div className="max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {(timeSlots && formData.consultationTime) && (
                    <div className="space-y-4">
                        <label className="flex items-center text-lg font-semibold text-gray-700">
                        ...
                        {loadingConsultants ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {timeSlots.map((consultant) => {
                                    const isAvailable = formData.consultationTime?.split(' ')[0] ===
                                    ...
                                    return (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                                handleConsultantSelect(consultant.bookingUuid)
                                            }}
                                            className={`
                                                relative p-4 rounded-lg border-2 text-left transition-all duration-200 transform hover:scale-105
                                            `}
                                        >
                                                ...
                                                {isAvailable ? (
                                                    ...
                                                    <div className="space-y-1">
                                                        <p className="text-xs text-gray-500">
                                                            Có sẵn lúc {new Date(consultant.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                        </p>
                                                    </div>
                                                )}
                                            </div>
